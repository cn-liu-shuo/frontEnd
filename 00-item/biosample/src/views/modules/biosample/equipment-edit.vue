  ...
  <div>
    <el-row style="margin-top: -10px">
      <el-col>
        ...
        <el-button type="primary" icon="el-icon-c-scale-to-original" @click="showForzenshelfBatch">生成分区(冻存架)</el-button>
        <el-button type="primary" icon="el-icon-c-scale-to-original" @click="showForzenboxBatch">生成冻存盒</el-button>
      </el-col>
    </el-row>
    ...
    <el-row :gutter="5" style="margin-top: 30px">
      <el-col :span="24">
        ...
        <div style="float: left">
          <el-tag type="warning" size="mini">1. 冰箱分区(冻存架)</el-tag>
          <el-button type="primary" @click="shelfAddOrUpdateHandle" icon="el-icon-edit">编辑</el-button>
          <div :style="{width: (boxColumnNums*60+20-(boxColumnNums-3)) + 'px', height: (boxRowNums*90+20-(boxRowNums-3)) + 'px', border: '1px solid #7DABD9'}">
            <ul class="box" :style="{width: (boxColumnNums*60+20-(boxColumnNums-2)) + 'px', height: (boxRowNums*90+20-(boxRowNums-2)) + 'px', marginLeft: '1px'}">
              ...
              <template v-for="i in this.boxRowNums">
                <li style="background-color: #D7DFE9; width: 20px;"><a style="height: 90px; width: 20px">{{ getLetter(i) }}</a></li>
                <template v-for="j in boxColumnNums">
                  <li v-if="selectRow==i && selectColumn==j" style="background-color: #7DABD9" @click="selectCell(i, j)">
                    <a style="height: 90px; word-break:break-all;">
                      {{ getForzenShelf(i, j) }}
                    </a>
                  </li>
                  <li v-else @click="selectCell(i, j)">
                    <a style="height: 90px; overflow: hidden;">
                      {{ getForzenShelf(i, j) }}
                    </a>
                  </li>
                </template>
          ...
          <img v-if="dataForm.item == 3" style="margin-left: -1px; margin-top: -1px" src="static/img/biosample/equipment/yd_bottom.png" :width="boxColumnNums*60+20-(boxColumnNums-6)" />
          <img v-else style="margin-left: -1px; margin-top: -1px" src="static/img/biosample/equipment/bottom.png" :width="boxColumnNums*60+20-(boxColumnNums-6)" />
        ...
        <div v-if="this.selectShelfItem" style="float: left; margin-left: 30px">
          <el-tag type="success" size="mini">2. 冻存盒</el-tag>
        ...
        <div v-if="this.selectshelfBoxItem" style="float: left; margin-left: 30px; margin-top: 20px">
          <el-tag type="success" size="mini">3. 样本</el-tag>
    ...
    <!-- 弹窗, 新增 / 修改 -->
    <shelf-add-or-update v-if="shelfAddOrUpdateVisible" ref="shelfAddOrUpdate"></shelf-add-or-update>
    <!-- 弹窗, 新增 / 修改 -->
    <box-add-or-update v-if="boxAddOrUpdateVisible" ref="boxAddOrUpdate"></box-add-or-update>
    <!-- 弹窗, 新增 / 修改 -->
  ...
  export default {
    ...
    methods: {
        ...
        if (this.dataForm.id) {
          equipmentApi.getEquipmentDetailInfo(this.dataForm.id).then(({data}) => {
            if (data && data.code === 0) {
              ...
              this.boxColumnNums = this.dataForm.columnNums
      ...
      selectCell (x, y) {
        this.selectRow = x
        this.selectColumn = y
        this.selectShelfItem = this.getForzenShelfRow(x, y)
        if (this.selectShelfItem) {
