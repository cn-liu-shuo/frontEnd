<template>
  <el-dialog
    ...
    <el-form-item label="位置编码" prop="encodingType">
      ...
      <!--<el-input v-model="dataForm.encodingType" placeholder="位置编码：1纯数字 2混合"></el-input>-->
    </el-form-item>
    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/biosample/forzenshelftype/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.name = data.forzenShelfType.name
                this.dataForm.rowNums = data.forzenShelfType.rowNums
                this.dataForm.columnNums = data.forzenShelfType.columnNums
                this.dataForm.encodingType = data.forzenShelfType.encodingType
                this.dataForm.width = data.forzenShelfType.width
                this.dataForm.height = data.forzenShelfType.height
                this.dataForm.createOn = data.forzenShelfType.createOn
                this.dataForm.createUserId = data.forzenShelfType.createUserId
                this.dataForm.createBy = data.forzenShelfType.createBy
                this.dataForm.modifiedOn = data.forzenShelfType.modifiedOn
                this.dataForm.modifiedUserId = data.forzenShelfType.modifiedUserId
                this.dataForm.modifiedBy = data.forzenShelfType.modifiedBy
                this.dataForm.remark = data.forzenShelfType.remark
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/biosample/forzenshelftype/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
