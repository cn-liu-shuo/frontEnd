          ...
          <el-col :span="12">
            <el-form-item label="冻存盒" prop="boxId">
              <el-input :readonly="true" v-model="dataForm.boxName" placeholder="选择冻存盒">
                <el-button slot="append" icon="el-icon-search" @click="querySamplesTransportSelectBox()"></el-button>
              </el-input>
            </el-form-item>
          </el-col>
        </el-row>
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item label="第X行" prop="positionX">
              <el-input-number :min="0" :max="dataForm.xMax" v-model="dataForm.positionX" style="width: 100%" placeholder="冻存管位置；格式(x;y)代表第几列 Y代表第几行  X"></el-input-number>
            </el-form-item>
          </el-col>

          <el-col :span="12">
            <el-form-item label="第Y列" prop="positionY">
              <el-input-number :min="0" :max="dataForm.yMax" v-model="dataForm.positionY" style="width: 100%" placeholder="冻存管位置；格式(x;y)代表第几列 Y代表第几行  X"></el-input-number>
            </el-form-item>
          </el-col>
        </el-row>
      </el-form>
  ...
  export default {
    ...
    data () {
      return {
        ...
        dataRule: {
          ...
          positionX: [
                { required: true, message: '行位置不能为空', trigger: 'blur' }
          ],
          positionY: [
                { required: true, message: '列位置不能为空', trigger: 'blur' }
          ]
        }
      }
    },
    methods: {
      ...
      selectEquipment (row) {
        ...
        this.dataForm.shelfId = null
        this.dataForm.shelfName = null
        this.dataForm.boxId = null
        this.dataForm.boxName = null
        this.dataForm.positionX = 0
        this.dataForm.positionY = 0
      },
      selectShelf (row) {
      ...
      selectBox (row) {
        ...
        this.dataForm.positionY = 0
          ...
          // 根据冻存盒id获取盒子行列值
        this.$http({
          url: this.$http.adornUrl(`/biosample/forzenbox/info/${this.dataForm.boxId}`),
          method: 'post',
          data: this.$http.adornData(this.dataForm.shelfId, false)
        }).then(({data}) => {
          if (data && data.code === 0) {
            this.dataForm.xMax = parseInt(data.forzenBox.rowNums)
            this.dataForm.yMax = parseInt(data.forzenBox.columnNums)
          } else {
      ...
      querySamplesTransportSelectBox () {
